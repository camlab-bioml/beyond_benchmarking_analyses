...
designMat <- designMat %>%
  filter(sil != "clustering failed") %>%
  filter(db != "clustering failed") %>%
  filter(ch != "clustering failed") %>%
...
saveRDS(designMat, "/home/campbell/cfang/bb-rebuttal/data/large_samples_corrected_designMat_unscaled.RDS")
