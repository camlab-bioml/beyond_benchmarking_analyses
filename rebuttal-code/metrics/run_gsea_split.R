...
whichClust <- args[[3]]
pathways.hallmark <- gmtPathways("code/c5.all.v7.4.symbols.gmt")
l <- lengths(pathways.hallmark)
pathways.hallmark <- pathways.hallmark[which(l > 10 & l < 500)]


exp.name <- strsplit(sce_name, "/")[[1]][[3]]
print(exp.name)
...
if (length(unique(clusters[[whichClust]])) <= 1){
  ...
  gsea_filename <- paste("/home/campbell/cfang/bb-rebuttal/results/gsea_results", filename, sep="/")
  print(gsea_filename)
