...
trainDownsampleType <- trainMat$downsampleType
...
testDownsampleType <- testMat$downsampleType
...
print(any(is.na(trainScores$gseaScaledImp)))
print(var(trainScores$gseaScaledImp))
print(colnames(trainMat))
gseaModel <- train(x=trainMat, y=as.numeric(trainScores$gseaScaledImp), method="glmnet", trControl=control)
print(gseaModel)
print(colnames(testMat))
...
saveRDS(models, "/home/campbell/cfang/automl_scrna/results/models/linearRegression/linearRegressionPreds_withDownsampled.RDS")
