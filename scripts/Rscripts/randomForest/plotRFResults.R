...
RFType <- "Numeric"
if(is.factor(models$sil$trainData$res)){
  RFType <- "Factor"
}
...
filename <- sprintf("PaperFigures/May1RFPlots_%sParams.pdf", RFType)
pdf(filename)
...
silParams = sprintf("RF (mtry=%s, ntree=%s)", silRF$finalModel$mtry, sil$finalModel$ntree)
dbParams = sprintf("RF (mtry=%s, ntree=%s)", dbRF$finalModel$mtry, db$finalModel$ntree)
chParams = sprintf("RF (mtry=%s, ntree=%s)", chRF$finalModel$mtry, ch$finalModel$ntree)
gseaParams = sprintf("RF (mtry=%s, ntree=%s)", gseaRF$finalModel$mtry, gsea$finalModel$ntree)
...
ariHm <- ComplexHeatmap::Heatmap((as.matrix(corHmARI)), name="Prediction\ncorrelations\n(ARI)", left_annotation = col_ha, cluster_columns = FALSE)
...
ggplot(allARIdf, aes(x=metricf, y=V1, fill=metric))+
  geom_boxplot()+
  scale_fill_discrete(breaks=c("CH", "DB", "SIL", "GSEA"))+
  xlab("Metric")+
...
corHm <- ComplexHeatmap::Heatmap((as.matrix(corHmDf)), name="Prediction\ncorrelations\n(ground truth)", left_annotation = col_ha,cluster_columns = FALSE)

ht_list <- corHm %v% ariHm
draw(ht_list, ht_gap=unit(0.75, "cm"), row_title="Feature", column_title="Metric", column_title_side="bottom")
